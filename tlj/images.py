...
def print_resource(basedir, arc, path: pathlib.Path, res: ResObject, indent: int = 0):
    ...
    if res.rtype == 'Image':
        print(path, res.rtype, res.subtype, res.index, res.name, len(res.children))
        with io.BytesIO(res.data) as s:
            ...
            print('  ' * (indent + 1), path, imagefile)

            if not imagefile or imagefile in {b'noname.xmg', b'noname'}:
                return

            with arc.open(imagefile.decode('ascii'), 'rb') as f:
                img = decode_xmg(f.read())
                imagepath = images_path / path.parent / 'xarc'
                imagepath.mkdir(parents=True, exist_ok=True)
                Image.fromarray(img, 'RGBA').save((imagepath / imagefile.decode('ascii')).with_suffix('.png'))
